...
public class Curriculum
{
    ...
    public bool isDone()
    {
        return lesson * reductionPercentage >= 1;
    }
...
public class GlobalCurriculumController : MonoBehaviour
{
    ...
    public IEnumerator UpdateAll(float reward, int activeSkill)
    {
        ...
        if (!locked && activeCurriculum.IsStepCountReached(academy.stepCount))
        {
            ...
            if (activeCurriculum.IsRewardReached(reward))
            {
                ...
                //log curriculum data
                CSV log = new CSV(activeCurriculum.CurriculumName, activeCurriculum.lesson, academy.stepCount/5, newLog);
                log.SaveCSV();
                ...
                activeCurriculum.LessonReset(academy.stepCount);
                ...
                foreach (LocalCurriculumController curr in curriculumControllerList)
                {
                    ...
                    curr.SetLesson(activeCurriculum.lesson, activeSkill, _newExpertReward);
                }
    ...
    public void ResetCurriculumLearning(int skillToBeReset)
    {
        ...
        foreach (LocalCurriculumController curr in curriculumControllerList)
        {
            curr.SetLesson(curriculumToBeReset.lesson, skillToBeReset, 0);
        }
    }
